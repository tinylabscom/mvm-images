  ...
  mkConfigfile =
    {
      ...
      # non-base kernel posture keeps a subsystem the base posture cuts; the
      # caller's own requiredExtraDisables stay hard — exemptions never
      # weaken them.
      #
      ...
      # on whatever the arch's defconfig happened to pick. Enforced at eval
      # time below, before the olddefconfig enable guard re-checks it
    ...
    pkgs.buildPackages.runCommandCC "mvm-kernel-config"
      {
        ...
        disableList = pkgs.lib.concatStringsSep " " (
          pkgs.lib.subtractLists disableExemptions (
