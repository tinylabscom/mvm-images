# Rootless workload-guest kernel — slim custom Linux 6.12.
#
# = the shared base (`kernel/base.nix`) + the dm-verity and generic rootless
# capability delta. The builder never opens a
...
base.mkKernel {
  extraEnables = [
    ...
    # network namespace would invite packet-network fallbacks that are outside
  ...
  # the builder kernel:
  #
  #   NETFILTER   — no guest packet-network device exists. External sockets
  #                 are opened by the host endpoint through authenticated
  #                 FlowMux over AF_VSOCK, so no in-guest firewall is useful.
  ...
  #   NAMESPACES / CGROUPS stay enabled in this posture because a rootless
  #                 runtime needs them. NET_NS remains a required cut: rootless
  #                 processes share the guest's loopback-only network
  #                 namespace and use FlowMux over AF_VSOCK for external I/O.
  #
  ...
  extraDisables = [
    ...
    # workload — the guest has loopback plus AF_VSOCK and no tunnels — and
    ...
    "INET6_XFRM_TUNNEL"
    "INET6_TUNNEL"
    # 6-in-4 is likewise unreachable: a guest has no packet-network endpoint.
    # It is the v6 tunnel that carries no XFRM, so
  ...
  # They do not get a private network namespace or another packet-network
  # escape hatch; all external sockets cross the authenticated vsock session.
  requiredExtraDisables = [ "NET_NS" ];
}
